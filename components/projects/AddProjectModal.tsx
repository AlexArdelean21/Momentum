        ...
        <div className={"fixed inset-0 z-50 flex " + (isMobile ? "items-end" : "items-center") + " justify-center p-0 md:p-4"} role="dialog" aria-modal="true" onKeyDown={onKeyDown}>
          ...
          >
            <div className="sticky top-0 z-10 bg-[var(--modal-bg,#111827)]/90 backdrop-blur supports-[backdrop-filter]:bg-[var(--modal-bg,#111827)]/70 border-b border-white/5 px-4 py-3 md:px-6">
              ...
              {isMobile && (
                <div className="mt-3">
                  <div className="flex space-x-2 rounded-xl bg-white/5 p-1 border border-white/10" role="tablist" aria-label="Project form steps">
                    <button type="button" role="tab" aria-selected={step === "details"} onClick={() => setStep("details")} className={(step === "details" ? "bg-gradient-to-r from-orange-500 to-rose-500 text-white " : "text-gray-300 hover:text-white ") + "flex-1 rounded-lg px-3 py-1.5 text-sm font-medium focus:outline-none focus-visible:ring-2"}>Details</button>
                    <button type="button" role="tab" aria-selected={step === "subtasks"} onClick={() => setStep("subtasks")} className={(step === "subtasks" ? "bg-gradient-to-r from-orange-500 to-rose-500 text-white " : "text-gray-300 hover:text-white ") + "flex-1 rounded-lg px-3 py-1.5 text-sm font-medium focus:outline-none focus-visible:ring-2"}>Subtasks</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <form className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 md:px-6 py-4 md:py-6 space-y-6" onSubmit={(e) => { e.preventDefault(); onSubmit() }}>
              ...
              <div className={isMobile && step !== "details" ? "hidden md:block" : ""}>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Choose an Emoji</label>
                <div>
                  <div className={"transition-all overflow-y-auto overscroll-contain border-2 border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50/50 dark:bg-gray-700/20 p-3 " + (emojiExpanded ? "max-h-80 md:max-h-96" : "max-h-36 md:max-h-40")}>
                  ...
                  {isMobile && (
                    <div className="mt-2">
                      <Button type="button" variant="ghost" size="sm" onClick={() => setEmojiExpanded((e) => !e)}>{emojiExpanded ? "Show less" : "Show more"}</Button>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className={isMobile && step !== "subtasks" ? "hidden md:block" : ""}>
                ...
                <div className="space-y-2">
                  {subtasks.map((s, i) => (
                    <ProjectSubtaskRow key={s.id || i} value={s} onChange={(patch) => updateSubtask(i, patch)} onRemove={() => removeSubtask(i)} disableRemove={subtasks.length <= 1} compact={!!isMobile} />
                  ))}
                </div>
              </div>
              ...
              <div className="sticky bottom-0 z-10 bg-[var(--modal-bg,#111827)]/90 backdrop-blur border-t border-white/5 px-0 md:px-0 pt-2 pb-3" style={{ paddingBottom: "calc(0.75rem + env(safe-area-inset-bottom))" }}>
                <div className="flex space-x-4">
                  ...
                  <Button type="submit" disabled={isPending || !isValid} className="flex-1 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white rounded-xl h-12 font-semibold shadow-lg hover:shadow-xl transition-all duration-300 border-0">Create</Button>
                </div>
              </div>
